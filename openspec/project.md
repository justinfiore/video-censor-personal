...
### `segment-allow-override` - Segment Allow Override

Add the ability for users to mark segments as "allowed" by manually adding `"allow": true` to a segment in the output JSON file. This provides a mechanism to handle false positives and make informed tradeoff decisions without re-analyzing the video.

**Rationale:**
The reason to add a property to the segment rather than just deleting the segment is:
1. **Preserve original detections**: The detection data (labels, confidence, description, reasoning) is retained for future reference
2. **Easy to change decisions**: User can toggle between allowed/not-allowed without re-analyzing the entire video
3. **Audit trail**: Maintains a record of what was detected even if the user chose to allow it
4. **Reversibility**: If user changes their mind, they can simply remove or change the `allow` property

**Behavior:**
- If a segment has `"allow": true`, it is **ignored** for all downstream processing:
  - Audio remediation (silence/bleep) — segment audio is left untouched
  - Chapter generation — segment is not marked as a chapter/skip point
  - Video editing/segment removal — segment video is not blanked or cut
- If a segment does **not** have an `allow` property (or has `"allow": false`), it is treated as "not allowed" (i.e., should be remediated)
- Original detection data (labels, confidence, description, detections array) is preserved regardless of allow status
- The `allow` property is additive—it doesn't require re-analyzing the video
- Multiple segments can be marked as allowed independently

**Detailed Use Cases:**

| Use Case | Scenario | User Action |
|----------|----------|-------------|
| **False Positive - Nudity** | System detects a segment as `Nudity`, but it's actually just a close-up of someone's face on screen. The detector flagged it incorrectly. | User reviews segment, determines it's a false positive, marks `"allow": true` to prevent incorrect blanking/muting |
| **False Positive - Violence** | System detects `Violence` for an action scene that is actually just fast camera movement with no actual violence | User marks `"allow": true` after reviewing |
| **Informed Tradeoff - Profanity** | System correctly detects `Profanity`, but by remediating (bleeping/silencing) it, the dialogue becomes incomprehensible and the plot is lost | User makes an active decision to allow this profanity for context preservation |
| **Informed Tradeoff - Context** | System detects mild profanity that is integral to a character's development or a pivotal scene | User consciously decides the artistic value outweighs the desire to censor |
| **Selective Filtering - Violence** | System detects multiple `Violence` segments throughout a movie. User wants to keep some fight scenes but remove others based on intensity. | User marks some violence segments as `"allow": true` and leaves others to be remediated |
| **Selective Filtering - Mixed Labels** | A segment has both `Profanity` and `Violence` labels. User is okay with the violence but not the profanity. | User leaves segment as not-allowed (entire segment remediated). Future enhancement could allow per-label overrides. |

**Example JSON (Allowed Segment):**
```json
{
  "start_time": "00:48:25",
  "end_time": "00:48:26",
  ...
  "allow": true,
  "labels": ["Profanity"],
  "description": "Character uses mild profanity in emotional scene",
  "confidence": 0.85,
  "detections": [
    {
      "label": "Profanity",
      "confidence": 0.85,
      "reasoning": "Detected mild expletive in dialogue"
    }
  ]
}
```

**Example JSON (Not Allowed - Default):**
```json
{
  "start_time": "01:12:30",
  "end_time": "01:12:45",
  "duration_seconds": 15,
  "labels": ["Violence"],
  "description": "Intense fight scene with blood",
  "confidence": 0.92,
  "detections": [
    {
      "label": "Violence",
      "confidence": 0.92,
      "reasoning": "Physical combat with visible injuries"
    }
  ]
}
```
Note: No `allow` property means this segment will be remediated.
